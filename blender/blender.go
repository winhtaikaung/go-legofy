...
func Normal(bg image.Image, fg image.Image) *image.RGBA {
	blder := new(Blender)
	dst := blder.blend(bg, fg, func(c0, c1 fcolor.RGBAF64) fcolor.RGBAF64 {
		return blder.alphaComp(c0, c1)
	})
...
func (blder *Blender) blend(bg image.Image, fg image.Image, fn func(fcolor.RGBAF64, fcolor.RGBAF64) fcolor.RGBAF64) *image.RGBA {
	bgBounds := bg.Bounds()
	fgBounds := fg.Bounds()
	...
	parallel.Line(h, func(start, end int) {
		for y := start; y < end; y++ {
			for x := 0; x < w; x++ {
				...
				// fmt.Println(result.R*255, result.G*255, result.B*255, result.A*255)
...
func (blder *Blender) overLayeffect(color float64, overlay float64) float64 {
	if color < 33 {
		return overlay - 100
	} else if color > 233 {
		return overlay + 100
...
func (blder *Blender) alphaComp(bg, fg fcolor.RGBAF64) fcolor.RGBAF64 {
	fg.Clamp()
	fga := fg.A
